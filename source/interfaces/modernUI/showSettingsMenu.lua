	...
	local function showOptions(window)
		...
		elseif selectedWindow == 3 then
			...
			for i, v in next, credit.translations do 
				players[player]._modernUISelectedItemImages[1][#players[player]._modernUISelectedItemImages[1]+1] = addImage(community[v], "&70", x+12 + counter%2*180, y + 17 + math.floor(counter/2)*13, player)
				ui.addTextArea(id..902+counter, '<g>'..i, player, x+30 + counter%2*180, y + 14 + math.floor(counter/2)*13, nil, nil, 0xff0000, 0xff0000, 0, true)
				counter = counter + 1
			end
		...
		if not donate then
			ui.addTextArea(id..899, '', player, x + (window-1)*167, y + 197, 146, 15, 0xbdbdbd, 0xbdbdbd, 0.5, true)
		end
	end
	if not donate then
