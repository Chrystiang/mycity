modernUI.showHousePermissions = function(self)
	...
	for user in next, ROOM.playerList do
		if user ~= player and players[user] and players[user].inRoom then
			showTextArea(id..(896+i), user, player, x+4 + (i-1)%2*173, y + floor((i-1)/2)*15, nil, nil, -1, 0xff0000, 0, true, 
				function(player, i)
					...
					showTextArea(id..'930', '<p align="center"><ce>'..user..'</ce>\n<v><font size="10">«'..translate("permissions_"..mainAssets.housePermissions[userPermission], player)..'»</font></v>', player, x+4 + (i-1)%2*173, y + floor((i-1)/2)*15, 150, 70, 0x432c04, 0x7a5817, 1, true)
					local counter = 0
					for _ = -1, 1 do
						if _ ~= room.terrains[terrainID].settings.permissions[user] then
							showTextArea(id..(931+counter), translate('setPermission', player):format(translate('permissions_'..mainAssets.housePermissions[_], player)), player, x+4 + (i-1)%2*173, y + 25 + floor((i-1)/2)*15 + counter*15, nil, nil, 0x432c04, 0x7a5817, 0, true,
								function()
