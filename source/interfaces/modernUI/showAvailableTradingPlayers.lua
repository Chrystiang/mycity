	...
	for user in next, ROOM.playerList do
		if user ~= player and players[user] and players[user].dataLoaded and players[user].inRoom then
			showTextArea(id..(896+i), user, player, x+4 + (i-1)%2*173, y + floor((i-1)/2)*15, nil, nil, -1, 0xff0000, 0, true, 
				function(player, i)
					showTextArea(id..'930', '<p align="center"><ce>'..user..'</ce>', player, x+4 + (i-1)%2*173, y + floor((i-1)/2)*15, 150, 70, 0x432c04, 0x7a5817, 1, true)
					showTextArea(id..'931', translate('txt_openProfile', player), player, x+4 + (i-1)%2*173, y + 25 + floor((i-1)/2)*15, nil, nil, 0x432c04, 0x7a5817, 0, true,
						function()
							eventTextAreaCallback(0, player, 'modernUI_Close_'..id, true)
							openProfile(player, user)
						end)
					showTextArea(id..'932', translate('trade_invitePlayer', player), player, x+4 + (i-1)%2*173, y + 25 + floor((i-1)/2)*15 + 15, nil, nil, 0x432c04, 0x7a5817, 0, true,
						function()
							eventTextAreaCallback(0, player, 'modernUI_Close_'..id, true)
